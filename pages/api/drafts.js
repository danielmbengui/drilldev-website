...
import { DIR_MIDJOURNEY_DATAS, DIR_MIDJOURNEY_DRAFTS, GALLERY_MAX_PICTURES_PER_PAGE, METHOD_GET, METHOD_POST, } from '@/constants';
...
function updateFile() {
    ...
    const _array = array.map((item,index) => {
        const _item = item;
        _item.id = index;
...
function getListPicturesBySearch(search, page, per_page) {
    ...
    const searchs = getRandomSortPictures(array.filter((item) => {
    ...
    const min = (_page - 1) * per_page;
    const max = (_page) * per_page;
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === METHOD_GET) {
            ...
            } else if (req.query.action === "get_list_pictures") {
                const search = req.query.search ? req.query.search : '';
                const page = req.query.page ? req.query.page : 1;
                const per_page = req.query.per_page ? req.query.per_page : GALLERY_MAX_PICTURES_PER_PAGE;
